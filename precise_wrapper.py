...
class PreciseEngine(Engine):
    ...
    def get_prediction(self, chunk):
        if len(chunk) != self.chunk_size:
            raise ValueError('Invalid chunk size: ' + str(len(chunk)) +". Expected="+str(self.chunk_size))
...
def runPrecise():
    ...
    #model = "/home/pi/precise_runner2/hey-mycroft.pb"
    model = "kodi-hotword.pb"
    engine = PreciseEngine(bin, model)
    precise = PreciseRunner(engine, on_prediction=on_prediction, on_activation=on_activation,
                  trigger_level=0)
    precise.start()
