...
try:
    ...
    raise sr.RequestError("bad PocketSphinx installation; try reinstalling PocketSphinx version 0.0.9 or better.")
if not hasattr(pocketsphinx, "Decoder") or not hasattr(pocketsphinx.Decoder, "default_config"):
    raise sr.RequestError("outdated PocketSphinx installation; ensure you have PocketSphinx version 0.0.9 or better.")
...
class MyRecognizer(sr.Recognizer): 
    ...
    def prepare_sphinx2(self, language="en-US", keyword_entries=None ):
        ...
        if isinstance(language, str):  # directory containing language data
            ...
            if not os.path.isdir(language_directory):
                raise sr.RequestError("missing PocketSphinx language data directory: \"{}\"".format(language_directory))
        ...
        if not os.path.isdir(acoustic_parameters_directory):
            raise sr.RequestError("missing PocketSphinx language model parameters directory: \"{}\"".format(acoustic_parameters_directory))
        if not os.path.isfile(language_model_file):
            raise sr.RequestError("missing PocketSphinx language model file: \"{}\"".format(language_model_file))
        if not os.path.isfile(phoneme_dictionary_file):
            raise sr.RequestError("missing PocketSphinx phoneme dictionary file: \"{}\"".format(phoneme_dictionary_file))
    ...
    def recognize_sphinx2(self, audio_data, onlykeywords):
        """
        ...
        if onlykeywords:  # explicitly specified set of keywords            
            self.decoder.start_utt()  # begin utterance processing
            #print("Run")
            self.decoder.process_raw(raw_data, False, True)  # process audio data with recognition enabled (no_search = False), as a full utterance (full_utt = True)
